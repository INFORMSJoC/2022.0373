function [ sensitivity ] = sensMRI( time )

if ( time == 1 )
sensitivity = 0.770; %adjustment due to MAMMOGRAPHY data
end

if ( time >= 2 && time <= 10 )
    sensitivity = 0.770; % no direct data, see #2 Leach et al
end

if ( time >= 11 && time <= 25 )
    sensitivity = 0.770; % see #2 Leach et al
end

if ( time >= 26 && time <= 35 )
    sensitivity = 0.91; % see #4 Kuhl et al
end

if ( time >= 36)
    sensitivity = 0.86; % see #5 Hagen et al
end
...
%                                       sensitivity of MRI 79.5% 
%                                       specificity of MRI 89.8%
%  (Overall) MRI Sensitivity 71.1%
